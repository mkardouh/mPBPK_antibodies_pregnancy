...
$PROB
A minimal PBPK model to predict fetal exposure to infliximab during pregnancy

$PARAM @annotated
// {Physiological Parameters}
GAinitial         : 0               : Gestational age (weeks) indicating simulation start time, defaults to 0 (beginning of pregnancy)
...
TVkef             : 0.021           : Fetal clearance term for infliximab (1/day)
  
// {Endosome 1 (Vascular) Parameters}
TVCLup            : 1.48            : Endothelial nonspecific pinocytosis rate (L/day) 
TVCLe             : 5.75472         : Lysosome catabolism clearance (L/day)
TVkrec            : 127.7193        : Endosome recycling rate constant (1/day)
...
TVkoff            : 13132.8         : Antibody-FcRn complex (FcRnA) dissociation rate constant (1/day) in endosomes


// {PK Parameters}
//ka              :            : Absorption rate constant (1/day)
//BA              :            : Subcutaneous (SC) bioavailability
...
// {Endosome 2 Compartment - syncytiotrophoblast} 
...
FcRn_e1_0=49800; //Concentration of FcRn in endosome 1 (nM)

double GA = GAinitial + TIME/7; //Gestational age (weeks) indicating simulation start time, defaults to 0 (beginning of pregnancy)

// FcRn expression equation as a variable to be included in the differential equations
if (GA < 0) double M2 = 0;
...
double VisfT = Visf*0.65; //Volume of interstitial fluid/intracellular water in the tight tissues (L)
double VisfL = Visf*0.35; //Volume of interstitial fluid/intracellular water in the leaky tissues (L)
...
if (GA >= 0) Visf = ( 19.81 + 0.5941*GA - 0.007*pow(GA, 2) )* exp(ETA(17));  
...
$TABLE
// Define variables of interest (calculated post-simulation)
...
$PROB
A minimal PBPK to predict fetal exposure to adalimumab during pregnancy
...
TVkef             : 0.027           : Fetal clearance term for adalimumab (1/day)
...
TVkoff            : 13996.8         : Antibody-FcRn complex (FcRnA) dissociation rate constant (1/day) in endosomes

// {PK Parameters for Adalimumab}
TVka              : 0.343          : Absorption rate constant (1/day)
TVBA              : 0.64           : Subcutaneous (SC) bioavailability
...
$PROB
A minimal PBPK model to predict fetal exposure to etanercept during pregnancy
...
TVkef             : 0.198           : Fetal clearance term for etanercept (1/day)
...
TVkoff            : 11577.6         : Antibody-FcRn complex (FcRnA) dissociation rate constant (1/day) in endosomes


// {PK Parameters for etanercept}
TVka              : 0.7968   : Absorption rate constant (1/day)
TVBA              : 0.58     : Subcutaneous (SC) bioavailability
...
$PROB
A minimal PBPK model to predict fetal exposure to ustekinumab during pregnancy
...
TVRfL             : 0.697           : Vascular reflection coefficient for leaky tissues
TVRflymph         : 0.2             : Lymphatic vascular reflection coefficient
TVkef             : 0.030           : Fetal clearance term for ustekinumab (1/day)
...
TVkoff            : 8640            : Antibody-FcRn complex (FcRnA) dissociation rate constant (1/day) in endosomes
...
if (disease==0) double TVka = 0.142; //Absorption rate constant (1/day) for UC
else if (disease==1) TVka = 0.181; //Absorption rate constant (1/day) for CD

if (disease==0) double TVBA = 0.872; //Subcutaneous (SC) bioavailability for UC
else if (disease==1) TVBA = 0.78; //Subcutaneous (SC) bioavailability for CD
...
if (GA >= 0) Visf = ( 19.81 + 0.5941*GA - 0.007*pow(GA, 2) )* exp(ETA(16));  
...
$PROB
A minimal PBPK model to predict fetal exposure to vedolizumab during pregnancy
...
TVRfL             : 0.697           : Vascular reflection coefficient for leaky tissues
TVRflymph         : 0.2             : Lymphatic vascular reflection coefficient
TVkef             : 0.1             : Fetal clearance term for vedolizumab (1/day)
TVKm              : 6.566757        : Additional maternal non-linear clearance for vedolizumab. Km is concentration at half-maximum elimination rate (nM)
TVVmax            : 1.805177        : Additional maternal non-linear clearance for vedolizumab. Vmax is maximum elimination rate (nmoles/day)
...
simfun <- function(rep, data, model, mw, recover = "dose, Delivery, disease, pregnancy, ID_new, Drug") {
  ans <- mrgsim(
    model,
    data = data,
    recover = recover,
...
server <- function(input, output, session) {
  ...
  observeEvent(input$simulate, {
    ...
    set.seed(2345)
    isim <- seq(input$n_vp)
    sim_output <- lapply(isim, simfun, data = data_input, model = get(input$model_select), mw = mw_value) %>% bind_rows()
    ...
    efficacy_value <- input$efficacy
    toxicity_value <- input$toxicity
    ...
    if (nrow(sim_output) > 0) {
      ...
      output$conc_plot <- renderPlot({
        mf %>%
          ...
          geom_hline(yintercept = efficacy_value, color = "steelblue4", linetype = "dashed", size = 1) +
          geom_hline(yintercept = toxicity_value, color = "plum4", linetype = "dashed", size = 1) +
          theme_bw() +
          theme(
  ...
  observe({
    #sreq(input$model_select)
    route_choices <- if (input$model_select %in% c("Infliximab", "Vedolizumab")) {
      c("IV" = "IV")
    } else if (input$model_select %in% c("Adalimumab", "Etanercept", "Ustekinumab")) {
      c("IV" = "IV", "SC" = "SC")
    }
    
    updateSelectInput(session, "admin_route", choices = route_choices, selected = "IV")
  })
  
  observe({
    if (is.null(input$num_events) || input$num_events < 1) return()

        isolate({
      for (i in 1:input$num_events) {
        route_input <- paste0("cmt_", i)
        
        if (input$model_select %in% c("Infliximab", "Vedolizumab")) {
          updateSelectInput(session, route_input, choices = c("IV" = "IV"), selected = "IV")
        } else if (input$model_select %in% c("Adalimumab", "Etanercept", "Ustekinumab")) {
          updateSelectInput(session, route_input, choices = c("IV" = "IV", "SC" = "SC"))
        }
      }
    })
  })
